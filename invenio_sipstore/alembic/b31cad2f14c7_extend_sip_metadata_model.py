...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("schema", name="uq_sipstore_sipmetadatatype_schema"),
    )
    op.create_table(
        "sipstore_sipmetadata",
        sa.Column("created", sa.DateTime(), nullable=False),
        sa.Column("updated", sa.DateTime(), nullable=False),
        sa.Column("sip_id", sqlalchemy_utils.types.uuid.UUIDType(), nullable=False),
        sa.Column("type_id", sa.Integer(), nullable=False),
        sa.Column("content", sa.Text(), nullable=False),
        sa.ForeignKeyConstraint(
            ["sip_id"], ["sipstore_sip.id"], name="fk_sipmetadata_sip_id"
        ),
        sa.ForeignKeyConstraint(
            ["type_id"], ["sipstore_sipmetadatatype.id"], name="fk_sipmetadatatype_type"
        ),
    ...
    op.add_column("sipstore_sip", sa.Column("archived", sa.Boolean(name="archived")))
...
def downgrade():
    ...
    sip_mtypes = dict(
        iter(conn.execute(sa.select([SIPMetadataType.c.id, SIPMetadataType.c.name])))
    )
    ...
    sip_metadata_type_mapping = sa.case(sip_mtypes, value=SIPMetadata.c.type_id)

    # Migrate SIP metadata back to SIP table
    op.execute(
        SIP.update()
        .values(content=SIPMetadata.c.content, sip_format=sip_metadata_type_mapping)
