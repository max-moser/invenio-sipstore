...
def upgrade():
    ...
    if conn.execute(old_sip_metadata_count).scalar() > 0:
        sip_metadata_types_defined = sa.select([sa.func.count()]).select_from(
            SIPMetadataType
        )
        if conn.execute(sip_metadata_types_defined).scalar() == 0:
            ...
            for (sip_format,) in conn.execute(
                ...
                op.execute(
                    SIPMetadataType.insert().values(
                        title=sip_format, name=sip_format, format=sip_format
                    )
                )
            print(
                "Check your SIPMetadataType entries, to possibly update or "
        ...
        sip_mtypes = dict(
            iter(
                conn.execute(sa.select([SIPMetadataType.c.name, SIPMetadataType.c.id]))
            )
        )
        if not sip_mtypes:
            raise Exception(
                "There are no SIP Metadata Types defined. Cannot"
        ...
        old_sip_metadata = sa.select(
            ...
            from_obj=OldSIP.outerjoin(SIPMetadata, OldSIP.c.id == SIPMetadata.c.sip_id),
        ).where(
            sa.and_(OldSIP.c.sip_format.isnot(None), SIPMetadata.c.sip_id.is_(None))
        )
        ...
        insert_to_sip_metadata = SIPMetadata.insert().from_select(
            ["sip_id", "created", "updated", "type_id", "content"], old_sip_metadata
        )
...
def downgrade():
    ...
    if conn.execute(sa.select([multi_typed_metadata_sips])).scalar():
        raise Exception(
            "You have multiple types of SIP metadata for some "
            "SIPs. Automatic downgrade not possible..."
        )

    # Add SIP 'sip_format' and 'content' columns without the NULL constraint
    op.add_column("sipstore_sip", sa.Column("sip_format", sa.VARCHAR(length=7)))
